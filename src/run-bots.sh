#!/bin/bash

python main.py --logic Menang --email=test@email.com --name=JUGG --password=123456 --team etimo &
python main.py --logic Teleport --email=test1@email.com --name=stima1 --password=123456 --team etimo &
python main.py --logic Random --email=test2@email.com --name=stima2 --password=123456 --team etimo &
python main.py --logic Random --email=test3@email.com --name=stima3 --password=123456 --team etimo &